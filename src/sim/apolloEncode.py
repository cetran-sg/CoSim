...
def encode_POmsgs(objCount, objIDs, objType, objHeadings, objLats, objLons, objutmx, objutmy, objVelXs, objVelYs, objBBxs, objBBys, objBBzs, t, seq):
    """ Package and serialize actor snapshot data from the simulator into Apollo's Perception Obstacle message
        ...
        Args:
            objCount int : Number of objects/obstacles in current simulation snapshot
            objIDs list : Unique identifier assigned to each object/obstacle
            objType string : Type of object (vehicle, pedestrian, cones etc.)
            objHeadings float : Heading of each object
            objLats float : Latitude of each object
            objLons float : Longitude of each object
            objutmx float : UTM X-coordinate of each object
            objutmy float : UTM Y-coordinate of each object
            objVelXs float : Velocity of each object in the X-axis
            objVelYs float : Velocity of each object in the Y-axis
            objBBxs float : Bounding box dimension of each object in the X-axis
            objBBys float : Bounding box dimension of each object in the Y-axis
            objBBzs float : Bounding box dimension of each object in the Z-axis
            t float : Timestamp
...
def encode_IMUmsgs(avelX,avelY,avelZ, accX, accY, accZ, t, seq):
    """ Package and serialize ego vehicle IMU data from the simulator to Apollo's format
        ...
        Args:
            avelX float : Angular velocity in the X-axis
            avelY float : Angular velocity in the Y-axis
            avelZ float : Angular velocity in the Z-axis
            accX float : Acceleration in the X-axis
            accY float : Acceleration in the Y-axis
            accZ float : Acceleration in the Z-axis
            t float : Timestamp
            seq int : Sequence number
...
def encode_corrIMUmsgs(avelX,avelY,avelZ,eulX,eulY,eulZ,accX,accY,accZ,heading,t):
    """ Package and serialize ego vehicle Corrected IMU data from the simulator to Apollo's format
        ...
        Args:
            ...
            avelY float : Angular velocity in the Y-axis
            avelZ float : Angular velocity in the Z-axis
            eulX float : Euler angle in the X-axis
            eulY float : Euler angle in the Y-axis
            eulZ float : Euler angle in the Z-axis
            accX float : Acceleration in the X-axis
            accY float : Acceleration in the Y-axis
            accZ float : Acceleration in the Z-axis
            heading float : Heading (yaw)
            t float : Timestamp
...
def encode_BPmsgs(lat, lon, utmx, utmy, t, seq):
    ...
    Args:
        lat float : Latitude
        lon float : Longitude
        utmx float : UTM X-coordinate
        utmy float : UTM Y-coordinate
        t float : Timestamp
        seq int : Sequence number
...
def encode_GPSmsg(lat,lon,utmx,utmy,qw,qx,qy,qz,velx,vely,heading,t,seq):
    ...
    Args:
        ...
        velx float : Velocity in X-axis
        vely float : Velocity in Y-axis
...
def encode_Chamsgs(t,lat,lon,utmx,utmy,odo,heading,vel,throttle,brake,steering):
    ...
    Args:
        ...
        utmx float : UTM X-coordinate
        utmy float : UTM Y-coordinate
        odo float : Odometer reading
        heading float : Heading
        vel float : Absolute velocity
        throttle float : Throttle percentage
        brake float : Brake percentage
...
def encode_Posemsgs(lat,lon,utmx,utmy,qw,qx,qy,qz,velx,vely,heading,avelX,avelY,avelZ, accX, accY, accZ,t,seq):
    ...
    Args:
        ...
        vely float : Velocity in Y-axis
        heading float : Heading
        avelX float : Angular velocity in X-axis
        avelY float : Angular velocity in Y-axis
        avelZ float : Angular velocity in Z-axis
        accX float : Acceleration in X-axis
        accY float : Acceleration in Y-axis
        accZ float : Acceleration in Z-axis
        t float : Timestamp
        seq int : Sequence number
...
def ConvertPoints(carlaPoint,t):
    """ Convert Carla points from the pointcloud to the Protobuf PointXYZIT format
    
    Args:
        carlaPoint : Point from each Carla pointcloud frame
        t float : Timestamp
    ...
    """
    point = PointXYZIT()
    point.intensity,point.timestamp,point.x,point.y,point.z = [int(carlaPoint.intensity*100),t,carlaPoint.point.x,-carlaPoint.point.y,carlaPoint.point.z]
    return point
